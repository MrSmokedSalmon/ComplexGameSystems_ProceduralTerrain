...
public class VegetationSystem : MonoBehaviour
{
    ...
    public void Vegetate(HeightMapGen chunk, float plantBounds = 0f)
    {
        if(instance == null)
            instance = this;
        ...
        {
            Debug.LogError(MissingHeightMapMessage(chunk.name));
            return;
        }
        
        float[,] heightMap = chunk.chunkData.heightMap;
        float heightScalar = chunk.heightMulti;
        Vector3 position = chunk.gameObject.transform.position;

        Vegetate(heightMap, heightScalar, position, plantBounds);
    }
    
    public void Vegetate(float[,] heightMap, float heightScalar, Vector3 chunkPosition, float plantBounds)
    {
        // Gets the width and the height and ensures they are an odd number
        int width = width = heightMap.GetLength(0);
        width -= width % 2 == 0 ? 1 : 0;
        int height = height = heightMap.GetLength(1);
        ...
        spawnedPlants = VegetationSystem.SpawnPlants(plantData);
    }

    private string MissingHeightMapMessage(string objectName) => objectName + " has not had a height map generated. " +
                                                                 "\nPlease ensure this is generated before generating the vegetation";
    ...
    public static List<GameObject> SpawnPlants(PlantData[] plantDatas)
    {
        ...
        {
            if(instance.plants[plant.plantType].prefab == null)
            {
                Debug.LogError("Plant prefab is unassigned or missing!");
                continue;
            }
    ...
    public static PlantData[] GenerateChunkPlants(int width, int height, float[,] heightMap, Vector3 chunkPosition, float heightScalar, float plantBounds = 0f)
    {
        ...
        {
            for (int y = 1; y < height; y += instance.targetDensity)
            {
